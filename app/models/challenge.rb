class Challenge < ActiveRecord::Base
  ...
  def build_default_game
    ...
    game.game_ranks.build(:user => challenger, :player => challenger.players.find_by!(tournament_id: tournament), :position => 1, :confirmed_at => time)
    game.game_ranks.build(:user => defender, :player => defender.players.find_by!(tournament_id: tournament), :position => 2, :confirmed_at => time)
    game
  end
  ...
  def build_response_game(winner)
    ...
    if winner
      game.game_ranks.build(:user => defender, :player => defender.players.find_by!(tournament_id: tournament), :position => 1, :confirmed_at => Time.zone.now)
      game.game_ranks.build(:user => challenger, :player => challenger.players.find_by!(tournament_id: tournament), :position => 2)
    else
      game.game_ranks.build(:user => defender, :player => defender.players.find_by!(tournament_id: tournament), :position => 2, :confirmed_at => Time.zone.now)
      game.game_ranks.build(:user => challenger, :player => challenger.players.find_by!(tournament_id: tournament), :position => 1)
    end
    game
  end
